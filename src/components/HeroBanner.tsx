...
const slideImages = [banner1, banner2, banner3, bannerServis];
const slideOverlays = [
...
const slideHrefs = ["/kategori/tv-goruntu", "/kategori/beyaz-esya", "/kategori/klima-isitma", "/servis"];
...
export default function HeroBanner() {
  ...
  const slides = [
    { image: slideImages[0], badge: t("hero.badge1"), title: t("hero.title1"), subtitle: t("hero.subtitle1"), desc: t("hero.desc1"), cta: { label: t("hero.cta1"), href: slideHrefs[0] }, overlay: slideOverlays[0] },
    { image: slideImages[1], badge: t("hero.badge2"), title: t("hero.title2"), subtitle: t("hero.subtitle2"), desc: t("hero.desc2"), cta: { label: t("hero.cta2"), href: slideHrefs[1] }, overlay: slideOverlays[1] },
    { image: slideImages[2], badge: t("hero.badge3"), title: t("hero.title3"), subtitle: t("hero.subtitle3"), desc: t("hero.desc3"), cta: { label: t("hero.cta3"), href: slideHrefs[2] }, overlay: slideOverlays[2] },
    { image: slideImages[3], badge: t("hero.badge4"), title: t("hero.title4"), subtitle: t("hero.subtitle4"), desc: t("hero.desc4"), cta: { label: t("hero.cta4"), href: slideHrefs[3] }, overlay: slideOverlays[3] },
  ];

  const goTo = useCallback((idx: number) => { setDirection(idx > current ? 1 : -1); setCurrent(idx); }, [current]);
  const next = useCallback(() => { setDirection(1); setCurrent(prev => (prev + 1) % slides.length); }, [slides.length]);
  const prev = useCallback(() => { setDirection(-1); setCurrent(prev => (prev - 1 + slides.length) % slides.length); }, [slides.length]);

  useEffect(() => { const timer = setInterval(next, AUTOPLAY_MS); return () => clearInterval(timer); }, [next]);

  const slide = slides[current];

  const imageVariants = {
    enter: (d: number) => ({ x: d > 0 ? "100%" : "-100%", opacity: 0, scale: 1.1 }),
    center: { x: 0, opacity: 1, scale: 1, transition: { duration: 0.8, ease: [0.25, 0.1, 0.25, 1] as const } },
    exit: (d: number) => ({ x: d > 0 ? "-30%" : "30%", opacity: 0, scale: 0.95, transition: { duration: 0.6, ease: [0.25, 0.1, 0.25, 1] as const } }),
  };

  const textVariants = {
    hidden: { opacity: 0, y: 40, filter: "blur(8px)" },
    visible: (i: number) => ({ opacity: 1, y: 0, filter: "blur(0px)", transition: { delay: 0.3 + i * 0.12, duration: 0.6, ease: [0.25, 0.1, 0.25, 1] as const } }),
  };

  return (
    <section className="relative w-full h-[50vh] sm:h-[55vh] md:h-[65vh] lg:h-[70vh] overflow-hidden bg-foreground">
      <AnimatePresence custom={direction} mode="popLayout">
        <motion.div key={current} custom={direction} variants={imageVariants} initial="enter" animate="center" exit="exit" className="absolute inset-0">
      ...
      <div className="absolute bottom-5 left-1/2 -translate-x-1/2 flex items-center gap-2">
        {slides.map((_, i) => (
          <button key={i} onClick={() => goTo(i)} className="relative h-2 rounded-full overflow-hidden transition-all duration-300" style={{ width: i === current ? 32 : 8 }} aria-label={`Slide ${i + 1}`}>
            <span className="absolute inset-0 bg-white/40 rounded-full" />
            {i === current && (
              <motion.span className="absolute inset-0 bg-white rounded-full origin-left" initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: AUTOPLAY_MS / 1000, ease: "linear" }} />
            )}
          </button>
        ))}
      </div>
